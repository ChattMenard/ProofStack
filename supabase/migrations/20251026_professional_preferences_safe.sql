-- Professional Preferences - "Hard Nos" system (Safe version)
...
-- Index for lookups (safe create)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_prof_prefs_profile') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_prof_prefs_availability') THEN
...
DROP POLICY IF EXISTS "Users can view own preferences" ON professional_preferences;
DROP POLICY IF EXISTS "Users can update own preferences" ON professional_preferences;
DROP POLICY IF EXISTS "Users can insert own preferences" ON professional_preferences;
DROP POLICY IF EXISTS "Employers can view preferences" ON professional_preferences;
...
-- Function to auto-update updated_at timestamp (safe create)
...
DROP TRIGGER IF EXISTS professional_preferences_updated_at ON professional_preferences;
